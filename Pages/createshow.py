...
#########################################################
'''Creates new seat enteries into seatdet'''
#########################################################    
...
#########################################################
'''Gets new slot id and insterts new slot entry '''
######################################################### 
    ...
    with SSHTunnelForwarder(
        ...
        try:
            conn = pymysql.connect(host='127.0.0.1', user=sql_username,
        			...
        			port=tunnel.local_bind_port)
            cur=conn.cursor()
            cur.execute("select slot_id from slotdet ORDER BY slot_id DESC LIMIT 1")    #gets latest slot id
